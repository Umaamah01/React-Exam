  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/todos/new" className="btn btn-primary btn-sm">
          Add Todo
        </Link>
      </div>
      ...
      <ul className="space-y-2">
        {filteredTodos.map((todo) => (
          <li
            key={todo.id}
            className="p-4 bg-base-100 rounded shadow flex justify-between items-center"
          >
            <span className={todo.completed ? "line-through text-gray-400" : ""}>
              {todo.title}
            </span>
            <Link to={`/todos/${todo.id}`} className="btn btn-sm btn-outline">
              View
            </Link>
          </li>
        ))}
      ...
      <div className="flex justify-between mt-6">
        ...
        <button onClick={() => setPage((old) => old + 1)} className="btn btn-sm">
          Next →
        </button>
      </div>
    </div>
